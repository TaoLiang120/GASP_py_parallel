...
class VaspEnergyCalculator(object):
    ...
    def __init__(self, incar_file, kpoints_file, potcar_files, geometry, path_to_callscript="./", callscript = "callvasp"):
        '''
    ...
    def do_energy_calculation(self, organism, dictionary, key,
                              composition_space, log_writer, nproc = 1, comm=None):
        """
        ...
        try:
            ...
            if rank_local==0:
                logstr = f'Error running VASP on organism {organism.id} '
                log_writer.write_data(logstr)
            comm.Barrier()
            dictionary[key] = None
            return
        ...
        if rank_local == 0:
            ...
            try:
                ...
                logstr = f'Error reading structure of organism {organism.id} from CONTCAR '
                log_writer.write_data(logstr)
                isvalid = False
                dictionary[key] = None
            ...
            if isvalid:
                ...
                if not converged:
                    logstr = f'VASP relaxation of organism {organism.id} did not converge '
                    log_writer.write_data(logstr)
                    isvalid = False
                    dictionary[key] = None
            else:
            ...
            if isvalid:
                ...
                logstr = f'Setting energy of organism {organism.id} to {organism.id.epa} eV/atom '
                log_writer.write_data(logstr)
            ...
            for line in lines:
                if 'atom_style' in line:
                    ...
                    for i in range(len(infos)-1, -1, -1):
                        if len(infos[i])<=3: element_symbols.insert(0, infos[i])
                        else: break
    ...
    def do_energy_calculation(self, organism, dictionary, key,
        ...
        try:
            ...
            if rank_local==0:
                ...
                logstr = f'Error running LAMMPS on organism {organism.id} '
                log_writer.write_data(logstr)
            comm.Barrier()
            dictionary[key] = None
            return
        ...
        if rank_local == 0:
            ...
            try:
                ...
                logstr = f'Error reading structure of organism {organism.id} from LAMMPS '
                log_writer.write_data(logstr)
                isvalid = False
                dictionary[key] = None
            ...
            if isvalid:
                ...
                if epa < -50:
                    logstr = f'Discarding organism {organism.id} due to unphysically large energy: {epa} eV/atom'
                    log_writer.write_data(logstr)
                    isvalid = False
                    dictionary[key] = None
            else:
            ...
            if isvalid:
                ...
                organism.epa = epa
                logstr = f'Setting energy of organism {organism.id} to {organism.epa} eV/atom '
                log_writer.write_data(logstr)
                dictionary[key] = organism
            else:
                dictionary[key] = None
...
class PyLammpsEnergyCalculator(object):
    ...
    def do_energy_calculation(self, organism, dictionary, key,
        ...
        isvalid = True
        try:
            total_energy, relaxed_coords, types = self.calculate_with_pylammps(organism.id, input_script_path, job_dir_path, comm=comm)
        except:
            if rank_local==0:
                # write the output of a bad LAMMPS call to for the user's reference
                logstr = f'Error running PYLAMMPS on organism {organism.id} '
                log_writer.write_data(logstr)
            isvalid=False
            dictionary[key] = None
        ...
        if rank_local==0:
            if isvalid:
                ...
                if total_energy is None:
                    logstr = f'Error reading energy of organism {organism.id} from PYLAMMPS output '
                    log_writer.write_data(logstr)
                    isvalid = False
                    dictionary[key] = None
            else:
            ...
            if isvalid:
                try:
                    ...
                    logstr = f'Error reading structure of organism {organism.id} from PYLAMMPS '
                    log_writer.write_data(logstr)
                    isvalid = False
                    dictionary[key] = None
            else:
    ...
    def do_energy_calculation(self, organism, dictionary, key,
        ...
        try:
            ...
            if rank_local==0:
                ...
                logstr = 'Error running GULP on organism {} '.format(organism.id)
                log_writer.write_data(logstr)
        ...
        if rank_local==0:
            ...
            if conv_err_string in gulp_output and gradient_norm > 0.1:
                logstr = f'The GULP calculation on organism {organism.id} did not converge '
                log_writer.write_data(logstr)
                isvalid = False
                dictionary[key] = None
            ...
            if isvalid:
                ...
                try:
                    ...
                    logstr = f'Error reading structure of organism {organism.id} from GULP '
                    log_writer.write_data(logstr)
                    isvalid=False
                    dictionary[key] = None
            else:
            ...
            if isvalid:
                ...
                try:
                    ...
                    logstr = f'Error reading energy of organism {organism.id} from GULP '
                    log_writer.write_data(logstr)
                    isvalid=False
                    dictionary[key] = None
            else:
